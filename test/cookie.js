    ...
    describe('Интеграционное тестирование', () => {
        ...
        describe('Фильтрация', () => {
            it('выводить список cookie, имя или значение которых соответствует фильтру', () => {
                ...
                // fixing tests for hide/show solution not for delete/create table rows
                assert.equal(listTable.querySelectorAll('[style*=table-row]').length, 1);
            ...
            it('добавлять cookie в таблицу, только если значение cookie соответствует фильтру', () => {
                ...
                assert.equal(listTable.querySelectorAll('[style*=table-row]').length, 2);
            });
