...
function findAllPSiblings(where) {
    ...
    for (let element of where.childNodes) {
        if (element.nextSibling && element.nextSibling.localName === "p") {
            result.push(element);
        }
    }
...
function collectDOMStat(root) {
    ...
    function classesCount(element, classesObj) {
        ...
        for (let currentClass of elementClasses) {
            if (currentClass in classesObj) {
                classesObj[currentClass]++;
            } else {
                classesObj[currentClass] = 1;
            }
        }
    ...
    function tagsCount(element, tagsObj, skipElement) {
        if (!tagsObj) {
            tagsObj = {};
        }
    ...
    result.texts = textNodeCount(root);
